...
class TestPlacementViewModel: XCTestCase, PlacementViewModelCallback {
    ...
    func test_signal_load_start_event() throws {
        ...
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalLoadStart", parentGuid: "placement1", jwtToken: "placements-token")))
        } else {
            XCTFail("No event")
        }
    }
    ...
    func test_signal_load_complete_event() throws {
        ...
        let result = XCTWaiter.wait(for: [exp], timeout: 0.3)
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalLoadComplete", parentGuid: "placement1", jwtToken: "placements-token")))
        } else {
            XCTFail("No event")
        }
    }
    ...
    func test_placement_impression_event() throws {
        ...
        let result = XCTWaiter.wait(for: [exp], timeout: 0.3)
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalImpression", parentGuid: "placement1", jwtToken: "placements-token")))
            let event = events[events.lastIndex(of: EventModel(eventType: "SignalImpression", parentGuid: "placement1", jwtToken: "placements-token"))!]
            ...
            XCTAssertTrue(event.containValueInMetadata(value: BE_RENDER_ENGINE_PLACEMENTS))
        } else {
            XCTFail("No event")
        }
    }
    ...
    func test_slot_impression_event() throws {
        ...
        let result = XCTWaiter.wait(for: [exp], timeout: 0.3)
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalImpression", parentGuid: "Offer 1", jwtToken: "slot-1-token")))
        } else {
            XCTFail("No event")
        }
    }
    ...
    func test_signal_activation_event() throws {
        ...
        let result = XCTWaiter.wait(for: [exp], timeout: 0.3)
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalActivation", parentGuid: "placement1", jwtToken: "placements-token")))
        } else {
            XCTFail("No event")
        }
    }
    ...
    func test_dismissal_end_message_event() throws {
        ...
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalDismissal",
                                                     parentGuid: "placement1",
                                                     jwtToken: "placements-token")))
    ...
    func test_go_to_next_offer_trigger_next_offer_impression() throws {
        ...
        let result = XCTWaiter.wait(for: [exp], timeout: 0.3)
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalImpression", parentGuid: "Offer 2", jwtToken: "slot-2-token")))
        } else {
            XCTFail("No event")
        }
    }
    ...
    func test_go_to_next_offer_skip_gost_offers() throws {
        ...
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalImpression", parentGuid: "Offer 3", jwtToken: "slot-3-token")))
        } else {
            XCTFail("No event")
        }
    }
    ...
    func getOffers() -> [OfferViewData] {
        return [
            OfferViewData(instanceGuid: "Offer 1", slotJWTToken: "slot-1-token", creativeJWTToken: "creative-token"),
            OfferViewData(instanceGuid: "Offer 2", slotJWTToken: "slot-2-token", creativeJWTToken: "creative-token"),
            OfferViewData(instanceGuid: "Offer 3", slotJWTToken: "slot-3-token", creativeJWTToken: "creative-token")
        ]
    }
