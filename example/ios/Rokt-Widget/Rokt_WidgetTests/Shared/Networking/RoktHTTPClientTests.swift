...
final class RoktHTTPClientTests: XCTestCase {
    ...
    func test_getFromURL_failsOnAllInvalidRepresentationCases() {
        XCTAssertNotNil(startRequestWithErrorFor(data: nil, response: nil, error: nil))
        XCTAssertNotNil(startRequestWithErrorFor(data: nil, response: nonHTTPURLResponse(), error: nil))
        XCTAssertNotNil(startRequestWithErrorFor(data: anyData(), response: nil, error: nil))
        XCTAssertNotNil(startRequestWithErrorFor(data: anyData(), response: nil, error: anyNSError()))
        XCTAssertNotNil(startRequestWithErrorFor(data: nil, response: nonHTTPURLResponse(), error: anyNSError()))
        XCTAssertNotNil(startRequestWithErrorFor(data: nil, response: anyHTTPURLResponse(), error: anyNSError()))
        XCTAssertNotNil(startRequestWithErrorFor(data: anyData(), response: nonHTTPURLResponse(), error: anyNSError()))
        XCTAssertNotNil(startRequestWithErrorFor(data: anyData(), response: anyHTTPURLResponse(), error: anyNSError()))
        XCTAssertNotNil(startRequestWithErrorFor(data: anyData(), response: nonHTTPURLResponse(), error: nil))
    }
    ...
    func test_downloadFile_savesDownloadedFileToLocalDestination() throws {
        ...
        let receivedValues = startDownloadWithResultFor(destinationURL: destinationURL, data: data, response: response, error: nil)
    ...
    func test_downloadFile_doesNotDownloadFileOnError() throws {
        ...
        let receivedValues = startDownloadWithResultFor(destinationURL: destinationURL, data: data, response: response, error: error)
    ...
    private func startRequestWithValuesFor(data: Data?, response: URLResponse?, error: Error?, file: StaticString = #file, line: UInt = #line) -> (data: Data?, response: HTTPURLResponse?) {
    ...
    private func startRequestWithErrorFor(data: Data?, response: URLResponse?, error: Error?, file: StaticString = #file, line: UInt = #line) -> Error? {
        let result = startRequestWithResultFor(data: data, response: response, error: error, file: file, line: line)
    ...
    private func startRequestWithResultFor(data: Data?, response: URLResponse?, error: Error?, file: StaticString = #file, line: UInt = #line) -> RoktHTTPRequestResult {
    ...
    private func startDownloadWithResultFor(destinationURL: URL, data: Data?, response: URLResponse?, error: Error?, file: StaticString = #file, line: UInt = #line) -> RoktDownloadResult {
        RoktHTTPUrlProtocolStub.stub(data: data, response: response, error: error)
        let sut = makeSUT(file: file, line: line)
        let exp = expectation(description: "Wait for completion")
