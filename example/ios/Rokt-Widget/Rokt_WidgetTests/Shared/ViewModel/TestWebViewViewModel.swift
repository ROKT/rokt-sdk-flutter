...
class TestWebViewViewModel: XCTestCase {
    ...
    func test_urlErrorCancelled_shouldNoSendDiagnostic() {
        let viewModel = WebViewViewModel(url: URL(string: "https://www.rokt.com"), sessionId: "", campaignId: "")
        
        viewModel.sendWebViewDiagnostics(URL(string: "https://www.example.com"), error: NSError(domain: "URL", code: NSURLErrorCancelled))
        
        let exp = expectation(description: "Test after 0.1 seconds")
        let result = XCTWaiter.wait(for: [exp], timeout: 0.1)
        
        if result == XCTWaiter.Result.timedOut {
            XCTAssert(!errors.contains(kWebViewErrorCode))
        } else {
            XCTFail("logged webview error code -999")
        }
    }
}
