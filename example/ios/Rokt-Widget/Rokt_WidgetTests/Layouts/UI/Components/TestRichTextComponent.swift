...
final class TestRichTextComponent: XCTestCase {
    func test_rich_text() throws {
        let view = TestPlaceHolder(layout: LayoutSchemaUIModel.richText(try get_model()))
    ...
    func test_rich_text_with_state() throws {
        let view = TestPlaceHolder(layout: LayoutSchemaUIModel.richText(try get_state_model()))
        ...
        XCTAssertEqual(rawText.stateReplacedAttributedString.string, "1 ORDER Number:")
        let colors = rawText.stateReplacedAttributedString.attribute(.foregroundColor, at: 0, effectiveRange: nil) as? UIColor
    ...
    func get_model() throws -> RichTextUIModel {
    ...
    func get_state_model() throws -> RichTextUIModel {
        let transformer = LayoutTransformer(layoutPlugin: get_mock_layout_plugin())
    ...
    func get_dark_config_model() throws -> LayoutSchemaUIModel? {
        let transformer = LayoutTransformer(layoutPlugin: get_mock_layout_plugin(), config: RoktConfig.Builder().colorMode(.dark).build())
