...
final class TestCreativeResponseViewModel: XCTestCase {
    ...
    func test_send_signal_impression_event() throws {
        ...
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalResponse", parentGuid: "creativeInstance")))
        } else {
            XCTFail("No event")
        }
    }
    ...
    func test_send_signal_gated_impression_event() throws {
        ...
        if result == XCTWaiter.Result.timedOut {
            XCTAssertTrue(events.contains(EventModel(eventType: "SignalGatedResponse", parentGuid: "creativeInstance")))
        } else {
            XCTFail("No event")
        }
    }
    ...
    func get_base_di() -> BaseDependencyInjection {
        return BaseDependencyInjection(sessionId: "session", pluginInstanceGuid: "instanceGuid", startDate: Date())
    }
