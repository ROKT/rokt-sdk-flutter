...
class BNFNodeMapper<DE: DataExtractor>: DomainMapper where DE.U == OfferModel {
    ...
    func map(
        ...
        case .progressIndicator(let indicatorModel):
            let updatedText = extractor.extractDataRepresentedBy(
                String.self,
    ...
    private func resolveDataExpansion(
        ...
        let placeholdersToResolved = placeholdersToResolvedValues(fullText, creativeParent: creativeParent, dataSource: dataSource)
    ...
    private func placeholdersToResolvedValues(
        ...
        for match in bnfMatches {
            ...
            let resolvedDataBinding = extractor.extractDataRepresentedBy(
                String.self,
