...
enum LayoutSchemaUIModel: Hashable {
    ...
    case zStack(ZStackUIModel)
    case scrollableRow(RowUIModel)
    case scrollableColumn(ColumnUIModel)
    case when(WhenUIModel)
    case oneByOne(OneByOneUIModel)
    case carousel(CarouselUIModel)
    case groupDistribution(GroupedDistributionUIModel)
    ...
    case progressControl(ProgressControlUIModel)
    case toggleButton(ToggleButtonUIModel)
    case empty
