...
struct BackgroundModifier: ViewModifier {
    ...
    var hasBackgroundColor: Bool {
        guard let backgroundColor = backgroundStyle?.backgroundColor,
              let color = backgroundColor.adaptiveColor, !color.isEmpty else {
            return false
        }
        return true
    }
    ...
    func body(content: Content) -> some View {
        content
            ...
            .backgroundColor(hex: backgroundStyle?.backgroundColor?.adaptiveColor)
    }   
}
...
struct BackgroundImageModifier: ViewModifier {
    ...
    var bgURL: ThemeUrl? { backgroundImage?.url }
    var bgScale: ContentMode { (backgroundImage?.scale ?? .fit).getScale() }
    ...
    func body(content: Content) -> some View {
        content.background(alignment: bgAlignment) {
            hasBackgroundImage ?
                AnyView(AsyncImageView(imageUrl: bgURL, scale: bgScale, isImageValid: $isImageValid)) :
                AnyView(EmptyView())
        }
        .clipped()
    }
