...
struct CreativeResponseViewModel {
    ...
    func sendSignalResponseEvent() {
        ...
        case .signalGatedResponse:
            baseDI.eventProcessor.sendGatedSignalResponseEvent(instanceGuid: model.responseOptions?.instanceGuid ?? "")
        case .signalResponse:
            baseDI.eventProcessor.sendSignalResponseEvent(instanceGuid: model.responseOptions?.instanceGuid ?? "")
        default:
            break
        }
    }
    ...
    func goToNextOffer() {
        baseDI.actionCollection["nextOffer"]()
    }
