...
internal extension Text {
    init(html: String, textColorHex: String?, uiFont: UIFont?, linkStyles: InlineTextStylingProperties?) {
        if let attributedString = try? html.htmlToAttributedString(textColorHex: textColorHex, uiFont: uiFont, linkStyles: linkStyles) {
...
internal extension StringProtocol {
    func htmlToAttributedString(
        ...
        return updateLinkStyles(linkStyles, attrStr: mutableMorphed)
    }

    private func updateLinkStyles(_ linkStyles: InlineTextStylingProperties?, attrStr: NSMutableAttributedString) -> AttributedString {
        let attrStrCopy = attrStr

        guard let linkStyles else { return AttributedString(attrStrCopy) }
        ...
        attrStrCopy.enumerateAttribute(.link, in: attrRange) { strValue, range, _ in
            ...
            setLinkColor(textColorHex: linkStyles.textColor?.adaptiveColor, originalStr: attrStrCopy, rangeToChange: range)
        ...
        return AttributedString(attrStrCopy)
    }
    ...
    private func setLinkFontProperties(
        ...
        originalStr.enumerateAttribute(.font, in: rangeToChange) { fontAtRange, fontRange, _ in
            ...
            if let fontSize {
                updatedFont = UIFont(descriptor: fDesc, size: CGFloat(fontSize))
            }
