...
internal class FontManager {
    ...
    static func reRegisterFonts(completionHandler: (() -> Void)? = nil) {
        ...
        for fontModel in fontsToDownload {
            ...
            guard let fileUrl = getFileUrl(name: registeredFontName) else {
                ...
                Log.i("Rokt: Error in font fileManager url")
                continue
            }
        ...
        if let fontData = try? NSData(contentsOf: fileUrl, options: [.mappedIfSafe]),
           let dataProvider = CGDataProvider(data: fontData) {
            if let cgFont = CGFont(dataProvider) {
                ...
                if Rokt.shared.initFeatureFlags.isEnabled(.shouldUseFontRegisterWithUrl) {
                ...
                Rokt.shared.isInitialized = false
                Rokt.shared.isInitFailedForFont = true
                Log.i("Rokt: Error on registering the font on device")
                RoktAPIHelper.sendDiagnostics(message: kAPIFontErrorCode,
            ...
            Rokt.shared.isInitialized = false
            Rokt.shared.isInitFailedForFont = true
            Log.i("Rokt: Error on reading the font data")
            RoktAPIHelper.sendDiagnostics(message: kAPIFontErrorCode,
    ...
    internal static func sendFullFontLogs(_ msg: String, fontLogId: String) {
        guard Rokt.shared.initFeatureFlags.isEnabled(.shouldLogFontHappyPath) else { return }
        Log.i("Rokt: \(msg)")
