...
class PlacementTransformer {
    ...
    init(placement: Placement, pageInstanceGuid: String, startDate: Date) {
    ...
    internal func transformOffer(slot: Slot, validIndex: Int) throws -> OfferViewData {
        if let offer = slot.offer {
            let totalValidOffer = getValidOfferCount()
            let content = try transferOfferTextViewData(text: try transferOfferContentText(slot: slot))
            var title: TextViewData?
            if let titleText = try transferOfferTitleText(slot: slot) {
                title = try transferOfferTitleTextViewData(titleText: titleText)
            }
            let positiveButton = try transferPositiveButton(offer: offer)
            let negativeButton = try transferNegativeButton(offer: offer)
            let imageVisibility: String? = try getOptionalString(MobileCreativeImageVisibility)
            let imageHideOnDark: Bool = imageVisibility != visible
            let image = ImageViewData(imageUrl:
                                        try transformCreativeImage(validIndex: validIndex,
                                                                   from: slot.offer?.creative.copy),
                                      imageHideOnDark: imageHideOnDark,
            ...
            return OfferViewData(instanceGuid: slot.instanceGuid,
                                 ...
                                 positiveButtonLabel: positiveButton,
                                 negativeButtonLabel: negativeButton,
                                 ...
                                 termsAndConditionsButton: try transformOfferTermsAndConditions(from:
                                                                                                    slot.offer?.creative.copy),
    ...
    private func transferOfferTextViewData(text: String) throws -> TextViewData {
    ...
    internal func transferOfferContentText(slot: Slot) throws -> String {
        let content = try getString(MobileCreativeCopy, from: slot.offer?.creative.copy)
        if let title = try getOptionalString(MobileCreativeTitle, from: slot.offer?.creative.copy),
           // if the CreativeTitleImageArrangement is not presented or it is .bottom, the title will
           // be attached to the content. Otherwise the title will be handeled in separate node
            try transferCreativeTitleImageArrangement() == .bottom {
            let isInLine = try getOptionalBool(MobileCreativeInLineCopyWithHeading) ?? true
            let titleEnding = isInLine ? " " : kBreakHTML
            
            return title + titleEnding + content
        }
        return content
    }
    ...
    internal func getEmptyTextStyle() -> TextStyleViewData {
        return TextStyleViewData(fontFamily: "", fontSize: 16, fontColor: ColorMap(), backgroundColor: nil)
    }
