...
internal extension NSMutableAttributedString {
    func setFontFace(font: UIFont, color: UIColor? = nil, fontWeight: UIFont.Weight? = nil) {
        beginEditing()
        ...
        self.enumerateAttribute(.font, in: NSRange(location: 0, length: self.length)) { (value, range, _) in
            if let f = value as? UIFont,
               let newFontDescriptor = f.fontDescriptor.withFamily(font.familyName)
                ...
                var newFont = UIFont(descriptor: newFontDescriptor, size: font.pointSize)

                if let fontWeight {
                    newFont = newFont.withWeight(fontWeight)
                }
