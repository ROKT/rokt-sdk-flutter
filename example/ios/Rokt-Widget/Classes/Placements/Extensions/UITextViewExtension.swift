...
internal extension UITextView {
    ...
    func set(textViewData: TextViewData?,
             collapseHeightAnchor: NSLayoutDimension? = nil,
            ...
            if let textViewData = textViewData {
                if let attributedString = textViewData.text.htmlAttributed {
                    ...
                    if let font = UIFont(name: style.fontFamily, size: CGFloat(style.fontSize)) {
                        text.setFontFace(
                            font: font,
                            color: UIColor(colorMap: style.fontColor, self.traitCollection),
                            fontWeight: style.fontWeight?.asUIFontWeight
                        )
                    } else {
                        // report diagnostics
                        RoktAPIHelper.sendDedupedFontDiagnostics(style.fontFamily)
                    }
                ...
                self.text = ""
                self.isHidden = true
                if let collapseHeightAnchor {
                    collapseHeightAnchor.constraint(equalToConstant: CGFloat(0)).isActive = true
                }
            }
            editingFinished?()
        }
