...
internal class RoktOfferView: UIView {
    ...
    @IBOutlet weak var offerTitleTextView: UITextView!
    ...
    override func didMoveToWindow() {
        ...
        // when the parent viewcontroller is removed
        if window == nil {
            clearSignalViewedTimers()
        }
    }
    ...
    internal func initialOfferView(viewController: UIViewController?,
        ...
        self.offerTitleTextView.textContainer.lineFragmentPadding = 0
        self.offerTitleTextView.textContainerInset = .zero
    ...
    internal func goToOffer(offerViewData: OfferViewData, isNewOffer: Bool = true) {
        ...
        if isNewOffer && (offerContainer.window != nil || isFullScreenWidget()) {
            offerViewModel.sendImpressionEvent(
                offerViewData.creativeInstanceGuid,
                creativeJWTToken: offerViewData.creativeJWTToken
            )
            onBecomingViewed { [weak self] in
                self?.offerViewModel.sendViewedEvent(
                    offerViewData.creativeInstanceGuid,
                    creativeJWTToken: offerViewData.creativeJWTToken
                )
            }
        ...
        let offerImageTitleView = OfferImageTitleView(imageViewData: offer.image,
                                                      titleViewData: offer.content?.imageTitle,
                                                      notifySizeChanged: {
                                                        self.resizeOfferView()
                                                      })
        offerImageTitleView.loadOfferImageTitleView(container: imageContainer)

        setOfferContentTitle()
        setOfferContent()
    }

    private func setOfferContentTitle() {
        offerTitleTextView.delegate = self

        self.offerTitleTextView.set(textViewData: self.offer.content?.title,
                                    collapseHeightAnchor: self.offerTitleTextView.heightAnchor)
    }

    private func setOfferContent() {
        offerTextView.delegate = self

        self.offerTextView.set(textViewData: self.offer.content?.body, editingFinished: {
            guard let content = self.offer.content?.body else { return }
            guard let font = UIFont(name: content.textStyleViewData.fontFamily,
                                    size: CGFloat(content.textStyleViewData.fontSize))
            else {
                // report diagnostics
                RoktAPIHelper.sendDedupedFontDiagnostics(content.textStyleViewData.fontFamily)
                return
            }
            ...
            if let termsLink = self.offer.termsAndConditionsButton {
                ...
                self.offerTextView.set(textViewData: self.offer.content?.body, extraString: termsAndPrivacyHtml,
                                       editingFinished: {
        ...
        if let privacyLink = self.offer.privacyPolicyButton {
            let finalTermsAndPrivacy = termsAndPrivacyHtml +
                self.convertLinkViewDatatoHTML(privacyLink, font: font,
                                               lastLineWidth: self.offerTextView.getLasLineWidth(),
                                               availableWidth: self.offerTextView.frame.width)
            self.offerTextView.set(textViewData: self.offer.content?.body, extraString: finalTermsAndPrivacy,
                                   editingFinished: {
    ...
    private func offerLoadingFinished() {
        self.offerTitleTextView.sizeToFit()
